...
public class NiceSpinner extends AppCompatTextView {
	...
	public <T> void attachDataSourceOriginal(List<T> list) {
	...
	private void setListViewValue() {
		...
		if (onItemClickListener != null) {
			onItemClickListener.onItemClick(listView, null, 0, 0);
		}
	}

	public <T> void removeDataSource(List<T> list, String str) {
		if (adapter != null) {
			if (list.size() == 0) {
				adapter.addItems(list);
				setTextInternal("");
			} else {
				if (adapter.getItemInDataset(selectedIndex).equals(str)) {
					adapter.addItems(list);
					setListViewValue();
				} else {
					adapter.addItems(list);
